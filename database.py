...
async def add_code(code, content_type, content):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "INSERT INTO codes (code, content_type, content) VALUES (?, ?, ?)",
...
async def add_stat(action):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "INSERT INTO statistics(action) VALUES (?)",
            (action,)
        )
        await db.commit()
...
async def get_stats_count():
    async with aiosqlite.connect(DB_NAME) as db:
        cursor = await db.execute("SELECT COUNT(*) FROM statistics")
        return (await cursor.fetchone())[0]
